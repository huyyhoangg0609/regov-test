...
let IndySdkPoolService = class IndySdkPoolService {
    constructor(indySdk, logger, stop$, fileSystem) {
        this.pools = [];
        this.logger = logger;
    ...
    setPools(poolConfigs) {
        this.pools = poolConfigs.map((poolConfig) => new IndySdkPool_1.IndySdkPool(poolConfig, this.indySdk, this.logger, this.stop$, this.fileSystem));
    }
    /**
    ...
    async getPoolForDid(agentContext, did) {
        ...
        if (pools.length === 0) {
            throw new error_2.IndySdkPoolNotConfiguredError("No indy ledgers configured. Provide at least one pool configuration in the 'indyLedgers' agent configuration");
        }
        const cache = agentContext.dependencyManager.resolve(core_1.CacheModuleConfig).cache;
        const cachedNymResponse = await cache.get(agentContext, `IndySdkPoolService:${did}`);
        ...
        if (successful.length === 0) {
            ...
            throw new error_2.IndySdkPoolError(`Unknown error retrieving did '${did}' from '${rejectedOtherThanNotFound.length}' of '${pools.length}' ledgers`, { cause: rejectedOtherThanNotFound[0].reason });
        }
    ...
    getPoolForNamespace(indyNamespace) {
        ...
        if (!pool) {
            throw new error_2.IndySdkPoolNotFoundError(`No ledgers found for IndyNamespace '${indyNamespace}'.`);
        }
        return pool;
    }
    ...
    async getDidFromPool(did, pool) {
        try {
            ...
            if ((0, error_1.isIndyError)(error, 'LedgerNotFound')) {
                throw new error_2.IndySdkPoolNotFoundError(`Did '${did}' not found on ledger ${pool.id}`);
            }
            else {
                throw (0, error_1.isIndyError)(error) ? new error_1.IndySdkError(error) : error;
            }
...
IndySdkPoolService = __decorate([
    ...
    __param(2, (0, core_1.inject)(core_1.InjectionSymbols.Stop$)),
    __param(3, (0, core_1.inject)(core_1.InjectionSymbols.FileSystem)),
    __metadata("design:paramtypes", [Object, Object, rxjs_1.Subject, Object])
], IndySdkPoolService);
