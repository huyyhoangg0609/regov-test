...
const util_1 = require("../../formats/indy/util");
...
class V1ProofProtocol extends BaseProofProtocol_1.BaseProofProtocol {
    ...
    async createProposal(agentContext, { proofFormats, connectionRecord, comment, parentThreadId, autoAcceptProof, }) {
        ...
        if (!proofFormats.indy) {
            throw new AriesFrameworkError_1.AriesFrameworkError('Missing indy proof format in v1 create proposal call.');
        }
        ...
        MessageValidator_1.MessageValidator.validateSync(presentationProposal);
        // Create message
    ...
    async processProposal(messageContext) {
        ...
        if (proofRecord) {
            ...
            await this.updateState(agentContext, proofRecord, ProofState_1.ProofState.ProposalReceived);
        }
        else {
    ...
    async acceptProposal(agentContext, { proofRecord, proofFormats, comment, autoAcceptProof }) {
        ...
        const requestFromPreview = (0, util_1.createRequestFromPreview)({
        ...
        const proposalAttachment = new Attachment_1.Attachment({
            data: {
                json: JsonTransformer_1.JsonTransformer.toJSON(requestFromPreview),
            },
        });
        // Create message
    ...
    async createRequest(agentContext, { proofFormats, connectionRecord, comment, parentThreadId, autoAcceptProof, }) {
        ...
        const didCommMessageRepository = agentContext.dependencyManager.resolve(storage_1.DidCommMessageRepository);
        if (!proofFormats.indy) {
            throw new AriesFrameworkError_1.AriesFrameworkError('Missing indy proof request data for v1 create request');
        }
        // Create record
        const proofRecord = new ProofExchangeRecord_1.ProofExchangeRecord({
    ...
    async processRequest(messageContext) {
        ...
        if (!requestAttachment) {
            throw new AriesFrameworkError_1.AriesFrameworkError(`Indy attachment with id ${messages_1.INDY_PROOF_REQUEST_ATTACHMENT_ID} not found in request message`);
        }
        ...
        if (proofRecord) {
            ...
            await this.updateState(agentContext, proofRecord, ProofState_1.ProofState.RequestReceived);
        }
        else {
            // No proof record exists with thread id
            proofRecord = new ProofExchangeRecord_1.ProofExchangeRecord({
    ...
    async negotiateRequest(agentContext, { proofFormats, proofRecord, comment, autoAcceptProof }) {
        ...
        if (!proofRecord.connectionId) {
            throw new AriesFrameworkError_1.AriesFrameworkError(`No connectionId found for proof record '${proofRecord.id}'. Connection-less verification does not support negotiation.`);
        }
        if (!proofFormats.indy) {
            throw new AriesFrameworkError_1.AriesFrameworkError('Missing indy proof format in v1 negotiate request call.');
        }
        const presentationProposal = new V1PresentationPreview_1.V1PresentationPreview({
            attributes: (_a = proofFormats.indy) === null || _a === void 0 ? void 0 : _a.attributes,
            predicates: (_b = proofFormats.indy) === null || _b === void 0 ? void 0 : _b.predicates,
    ...
    async acceptRequest(agentContext, { proofRecord, proofFormats, autoAcceptProof, comment }) {
        ...
        if (!requestAttachment || !indyProofRequest) {
            throw new errors_1.V1PresentationProblemReportError(`Missing indy attachment in request message for presentation with thread id ${proofRecord.threadId}`, { problemCode: PresentationProblemReportReason_1.PresentationProblemReportReason.Abandoned });
        }
        const proposalAttachment = proposalMessage
            ? new Attachment_1.Attachment({
                data: {
                    json: JsonTransformer_1.JsonTransformer.toJSON((0, util_1.createRequestFromPreview)({
    ...
    async getCredentialsForRequest(agentContext, { proofRecord, proofFormats }) {
        ...
        const indyProofRequest = requestMessage.indyProofRequest;
        if (!requestAttachment || !indyProofRequest) {
            throw new AriesFrameworkError_1.AriesFrameworkError(`Missing indy attachment in request message for presentation with thread id ${proofRecord.threadId}`);
        }
        const proposalAttachment = proposalMessage
            ? new Attachment_1.Attachment({
                data: {
    ...
    async processPresentation(messageContext) {
        ...
        if (!presentationAttachment) {
            throw new AriesFrameworkError_1.AriesFrameworkError('Missing indy proof attachment in processPresentation');
        }
        const requestAttachment = requestMessage.getRequestAttachmentById(messages_1.INDY_PROOF_REQUEST_ATTACHMENT_ID);
        if (!requestAttachment) {
            throw new AriesFrameworkError_1.AriesFrameworkError('Missing indy proof request attachment in processPresentation');
        }
        ...
        await this.updateState(agentContext, proofRecord, ProofState_1.ProofState.PresentationReceived);
        return proofRecord;
    }
    async acceptPresentation(agentContext, { proofRecord }) {
        ...
        // Update record
        await this.updateState(agentContext, proofRecord, ProofState_1.ProofState.Done);
    ...
    async shouldAutoRespondToProposal(agentContext, options) {
        ...
        if (!requestAttachment)
            return false;
        const rfc0592Proposal = JsonTransformer_1.JsonTransformer.toJSON((0, util_1.createRequestFromPreview)({
            name: 'Proof Request',
            nonce: await agentContext.wallet.generateNonce(),
            version: '1.0',
            attributes: proposalMessage.presentationProposal.attributes,
    ...
    async shouldAutoRespondToRequest(agentContext, options) {
        ...
        if (!proposalMessage)
            return false;
        const rfc0592Proposal = (0, util_1.createRequestFromPreview)({
            name: 'Proof Request',
            nonce: await agentContext.wallet.generateNonce(),
            version: '1.0',
            attributes: proposalMessage.presentationProposal.attributes,
        ...
        return this.indyProofFormat.shouldAutoRespondToRequest(agentContext, {
            ...
            proposalAttachment: new Attachment_1.Attachment({
                data: {
                    base64: utils_1.JsonEncoder.toBase64(rfc0592Proposal),
                },
            }),
            requestAttachment,
        });
    ...
    async shouldAutoRespondToPresentation(agentContext, options) {
        ...
        const rfc0592Proposal = proposalMessage
            ? JsonTransformer_1.JsonTransformer.toJSON((0, util_1.createRequestFromPreview)({
                name: 'Proof Request',
                nonce: await agentContext.wallet.generateNonce(),
                version: '1.0',
                attributes: proposalMessage.presentationProposal.attributes,
    ...
    async getFormatData(agentContext, proofRecordId) {
        ...
        if (proposalMessage && indyRequestProof) {
            indyProposeProof = (0, util_1.createRequestFromPreview)({
        ...
        return {
            proposal: proposalMessage
                ? {
                    indy: indyProposeProof === null || indyProposeProof === void 0 ? void 0 : indyProposeProof.toJSON(),
                }
                : undefined,
    ...
    assertOnlyIndyFormat(proofFormats) {
        ...
        if (formatKeys.length !== 1 || !formatKeys.includes('indy')) {
            throw new AriesFrameworkError_1.AriesFrameworkError('Only indy proof format is supported for present proof v1 protocol');
        }
    }
}
