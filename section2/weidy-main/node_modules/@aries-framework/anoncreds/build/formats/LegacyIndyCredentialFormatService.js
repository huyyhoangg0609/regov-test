...
class LegacyIndyCredentialFormatService {
    ...
    async createProposal(agentContext, { credentialFormats, credentialRecord }) {
        ...
        const proposal = new core_1.IndyCredPropose(indyCredentialProposal);
        try {
        ...
        const proposalJson = core_1.JsonTransformer.toJSON(proposal);
    ...
    async processProposal(agentContext, { attachment }) {
        ...
        core_1.JsonTransformer.fromJSON(proposalJson, core_1.IndyCredPropose);
    }
    async acceptProposal(agentContext, { attachmentId, credentialFormats, credentialRecord, proposalAttachment, }) {
        ...
        const credentialProposal = core_1.JsonTransformer.fromJSON(proposalAttachment.getDataAsJson(), core_1.IndyCredPropose);
        ...
        if (!credentialDefinitionId) {
            throw new core_1.AriesFrameworkError('No credentialDefinitionId in proposal or provided as input to accept proposal method.');
        }
        if (!attributes) {
    ...
    async processOffer(agentContext, { attachment, credentialRecord }) {
        ...
        if (!credOffer.schema_id || !credOffer.cred_def_id) {
            throw new core_1.CredentialProblemReportError('Invalid credential offer', {
    ...
    async acceptRequest(agentContext, { credentialRecord, attachmentId, offerAttachment, requestAttachment, }) {
        ...
        if (!credentialAttributes) {
            throw new core_1.CredentialProblemReportError(`Missing required credential attribute values on credential record with id ${credentialRecord.id}`, { problemCode: core_1.CredentialProblemReportReason.IssuanceAbandoned });
        }
    ...
    async processCredential(agentContext, { credentialRecord, attachment }) {
        ...
        if (!credentialRequestMetadata) {
            throw new core_1.CredentialProblemReportError(`Missing required request metadata for credential with id ${credentialRecord.id}`, { problemCode: core_1.CredentialProblemReportReason.IssuanceAbandoned });
        }
    ...
    async shouldAutoRespondToProposal(agentContext, { offerAttachment, proposalAttachment }) {
        ...
        const credentialProposal = core_1.JsonTransformer.fromJSON(credentialProposalJson, core_1.IndyCredPropose);
    ...
    getCredentialLinkedAttachments(attributes, linkedAttachments) {
        ...
        let previewAttributes = (_a = attributes === null || attributes === void 0 ? void 0 : attributes.map((attribute) => new core_1.CredentialPreviewAttribute(attribute))) !== null && _a !== void 0 ? _a : [];
        let attachments;
